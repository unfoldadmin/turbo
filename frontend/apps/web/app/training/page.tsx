...
export default function TrainingPage() {
  ...
  const expiredCerts = certifications.filter((c: any) => c.expiry_status === 'expired')
  const criticalCerts = certifications.filter((c: any) => c.expiry_status === 'critical')
  const warningCerts = certifications.filter((c: any) => c.expiry_status === 'warning')
  const cautionCerts = certifications.filter((c: any) => c.expiry_status === 'caution')
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-4">
        <Card className="p-6 bg-destructive/10 border-destructive/20">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <div className="text-2xl font-bold text-destructive">{expiredCerts.length}</div>
        ...
        <Card className="p-6 bg-destructive/10 border-destructive/20 border-2">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <div className="text-2xl font-bold text-destructive">{criticalCerts.length}</div>
        ...
        <Card className="p-6 bg-warning/10 border-warning/20">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <div className="text-2xl font-bold text-warning">{warningCerts.length}</div>
        ...
        <Card className="p-6 bg-warning/10 border-warning/20">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <div className="text-2xl font-bold text-warning">{cautionCerts.length}</div>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-border">
            <thead className="bg-muted/30">
              <tr>
                <th className="px-6 py-3 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider">Fueler</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider">Training</th>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider">Days Left</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider">Status</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider">Certified By</th>
                <th className="px-6 py-3 text-right text-xs font-medium text-muted-foreground uppercase tracking-wider">Actions</th>
              </tr>
            </thead>
            <tbody className="bg-card divide-y divide-border">
              {certifications.map((cert: any) => (
                <tr key={cert.id} className={`hover:bg-muted/10 ${cert.expiry_status === 'expired' ? 'bg-destructive/5' : ''}`}>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-foreground">{cert.fueler_name}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-muted-foreground">{cert.training_name}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-muted-foreground">{formatDate(cert.completed_date)}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-muted-foreground">{formatDate(cert.expiry_date)}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm">
                    <span className={`font-semibold ${cert.days_until_expiry < 0 ? 'text-destructive' : cert.days_until_expiry <= 3 ? 'text-destructive' : cert.days_until_expiry <= 7 ? 'text-warning' : 'text-foreground'}`}>
                      {cert.days_until_expiry < 0 ? `${Math.abs(cert.days_until_expiry)} days ago` : `${cert.days_until_expiry} days`}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <Badge className={getStatusBadge(cert.expiry_status)}>
                      {cert.expiry_status.charAt(0).toUpperCase() + cert.expiry_status.slice(1)}
                    </Badge>
                  </td>
