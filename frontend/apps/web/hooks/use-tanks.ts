...
export function useTanks() {
  ...
  const createTank = useCallback(async (tank: FuelTankRequest) => {
    const client = await getApiClient(session)
  ...
  const updateTank = useCallback(async (id: number, updates: FuelTankRequest) => {
    const client = await getApiClient(session)
  ...
  const deleteTank = useCallback(async (id: number) => {
    const client = await getApiClient(session)
