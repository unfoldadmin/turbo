...
export function ToolFallback(
    ...
    const prettyArgs = useMemo(() => {
        try {
            ...
            if (argsText && argsText.trim().length > 0) return argsText.trim();
        } catch { }
        return "";
    }, [args, argsText]);
    ...
    return (
        <details className="w-full max-w-xl rounded-md border border-gray-200 bg-white p-3 text-sm text-gray-900 shadow-sm">
            <summary className="flex cursor-pointer items-center justify-between">
                <span className="font-medium">{toolName}</span>
                <span className="text-xs text-gray-500">{status.type === "complete" ? "готово" : status.type}</span>
            </summary>
            <div className="mt-2 space-y-2">
                {prettyArgs && (
                    <div>
                        <div className="mb-1 text-xs text-gray-500">Аргументы</div>
                        <pre className="max-h-40 overflow-auto rounded bg-gray-50 p-2 text-xs text-gray-800">{prettyArgs}</pre>
                    </div>
                )}
                {result !== undefined && (
                    <div>
                        <div className="mb-1 text-xs text-gray-500">Результат</div>
                        <pre className="max-h-64 overflow-auto rounded bg-gray-50 p-2 text-xs text-gray-800">{prettyResult}</pre>
                    </div>
                )}
            </div>
        </details>
