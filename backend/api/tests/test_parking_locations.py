...
class ParkingLocationAPITests(APITestCase):
    ...
    def test_list_active_locations(self):
        ...
        active_count = sum(1 for loc in response.data['results'] if loc['display_order'] > 0)
    ...
    def test_list_all_locations(self):
        ...
        self.assertIn('results', response.data)
        self.assertGreaterEqual(len(response.data['results']), 3)  # At least our 3 test locations
    ...
    def test_filter_by_airport(self):
        ...
        self.assertIn('results', response.data)
        mso_locations = [loc for loc in response.data['results'] if loc['airport'] == 'MSO']
        self.assertGreaterEqual(len(mso_locations), 1)  # At least one MSO location from setUp
    ...
    def test_search_locations(self):
        ...
        terminal_results = [
            loc for loc in response.data['results']
    ...
    def test_update_location(self):
        ...
        response = self.client.patch(f'/api/parking-locations/{self.location1.id}/', data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_ordering(self):
        ...
        if len(test_locations) >= 2:
            t_a1 = next((loc for loc in test_locations if loc['location_code'] == 'T-A1'), None)
            usfs = next((loc for loc in test_locations if loc['location_code'] == 'USFS-1'), None)
            if t_a1 and usfs:
